...
def init_meeting_router(get_current_user, has_perm, require_perm, audit_log):
...
async def daily_huddle():
    ...
    bank = query_one("SELECT balance FROM bank_balance WHERE company_code = %s ORDER BY date DESC LIMIT 1", ("RM",))
    return {
